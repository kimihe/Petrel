...
class KVWorker : public SimpleApp {
 public:
   ...
   */
   ...
   void PushCoflow(FSCoflow* coflow,bool CoflowMethod);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 void MyThread0(KVWorker<float>* kv,std::vector<Key> keys,std::vector<float> vals,int repeat)
{
	std::vector<int> ts;
...
class FSFlow{
	public:
            FSFlow(){};
	    FSFlow(int server,KVWorker<float>* Worker,std::vector<Key> keys,std::vector<float> vals,int Size,clock_t Start,clock_t end,std::string KvApp);//构造函数
		//thread Send();     //启动线程，返回该线程
		void changeFlowInfoWithstart(clock_t start);    //改变flow信息
		void changeFlowInfoWithend(clock_t end);
		int GetFlowSize();    //获取flow信息
		KVWorker<float>* GetFlowWorker();
		std::vector<Key> GetFlowKeys();
		std::vector<float> GetFlowVals();
		clock_t GetFlowStart();
		clock_t GetFlowEnd();
		void description();
		bool complete;
	private:
	    KVWorker<float>* _worker;
		int _server;
		int _size;
		int _start;
		int _end;
...
class FSCoflow{
	public:
	    void AddFlow(FSFlow flow);  //为此Coflow添加flow
		//void CoflowStart();    //运行此coflow
		void removeflow();
		int GetFlowNumber();
		FSFlow* Getflow(int i);
		void PrintFlowtime();
	private:
	    FSFlow FlowMember[50];   //指向包含的flow的指针
		int FlowNumber=0;    //包含的flow数
		int start,end;
		int CoflowTimeConfused;   //此coflow的消耗时间
};


FSFlow::FSFlow(int server,KVWorker<float>* worker,std::vector<Key> keys,std::vector<float> vals,int size,clock_t start,clock_t end,std::string kvapp)
{
	//初始化flow信息
	_worker=worker;
	_keys=keys;
	_vals=vals;
	_size=size/GetBandwidth();
	_start=start;
	_kvapp=kvapp;
}

void FSFlow::changeFlowInfoWithstart(clock_t start)
{
	_start=start;
}
void FSFlow::changeFlowInfoWithend(clock_t end)
{
	_end=end;
}
int FSFlow::GetFlowSize()
{
    return _size;
}
KVWorker<float>* FSFlow::GetFlowWorker()
{
	return _worker;
}
std::vector<Key> FSFlow::GetFlowKeys()
{
	return _keys;
}
std::vector<float> FSFlow::GetFlowVals()
{
	return _vals;
}
clock_t FSFlow::GetFlowStart()
{
	return _start;
}
clock_t FSFlow::GetFlowEnd()
{
	return _end;
}

void FSCoflow::AddFlow(FSFlow flow)
{
	FlowMember[FlowNumber]=flow;
	FlowNumber++;
}
FSFlow* FSCoflow::Getflow(int i)
{
	return &FlowMember[i];
}
int FSCoflow::GetFlowNumber()
{
	return FlowNumber;
}
void FSCoflow::PrintFlowtime()
{
    int i=0;
	for(i=0;i<FlowNumber;i++)
	{
		if(FlowMember[i].complete)
		std::cout<<FlowMember[i].GetFlowStart()<<"  "<<FlowMember[i].GetFlowEnd()<<"  "<<FlowMember[i].GetFlowEnd()-FlowMember[i].GetFlowStart()<<std::endl;
	}
}
...
template <typename Val>
   void KVWorker<Val>::PushCoflow(FSCoflow* coflow,bool CoflowMethod)
   {
	   clock_t start[coflow->GetFlowNumber()],end[coflow->GetFlowNumber()];
	   int i=0,j=0;
	   FSFlow* flow[coflow->GetFlowNumber()];
	   for(i=0;i<coflow->GetFlowNumber();i++)
		   flow[i]=coflow->Getflow(i);
	   if(CoflowMethod)
	   {
		   std::thread *th[coflow->GetFlowNumber()];
           for(i=0;i<coflow->GetFlowNumber();i++)
		   {
			   start[i]=GetTickCount();
			   th[i]=new std::thread(MyThread0,this,flow[i]->GetFlowKeys(),flow[i]->GetFlowVals(),flow[i]->GetFlowSize());
		   }
		   for(i=0;i<coflow->GetFlowNumber();i++)
		   {
			   th[i]->join();
			   end[i]=GetTickCount();
			   flow[i]->complete=true;
		   }
		   for(i=0;i<coflow->GetFlowNumber();i++)
		   {
			   flow[i]->changeFlowInfoWithstart(start[i]);
			   flow[i]->changeFlowInfoWithend(end[i]);
		   }
	   }
	   if(!CoflowMethod)
	   {
	      ...
	      {       
		    std::vector<int> ts;
		    start[i]=GetTickCount();
			for(j=0;j<flow[i]->GetFlowSize();j++)
                        {
			    ts.push_back(this->Push(flow[i]->GetFlowKeys(),flow[i]->GetFlowVals()));
                            usleep(1000);
                         }
            for (int t : ts) this->Wait(t);
			end[i]=GetTickCount();
			flow[i]->complete=true;
	   }
	   for(i=0;i<coflow->GetFlowNumber();i++)
